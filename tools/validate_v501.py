#!/usr/bin/env python3
"""v5.01 regression checks for the dual-profile startup hotfix.
...
def check_static_pages(py_root: Path, report: CheckReport):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            os.chdir(old_cwd)
    report.check("static id=12 bundled page resolves outside project cwd", p12 is not None and p12.is_file())
    ...
    html = (py_root / "templates/static/12.html").read_text("utf-8")
    report.check("static id=12 is an age notice", "适龄提示" in html and "12+" in html)
    report.check("static route avoids cwd-only lookup", "Path(__file__).resolve()" in source)

    # Execute the actual async route body (with its decorator removed) so this
    # test covers the response status and type, not just path lookup.
    import fastapi
    ...
    report.check(
        "static id=12 route returns HTML 200",
        response12.status_code == 200 and response12.media_type.startswith("text/html"),
    )
    report.check(
        "unknown static route returns JSON 404",
...
def main() -> int:
    ...
    report.check("v5.01 build ID present", "v5.01-cn-gl-login-startup-hotfix" in build)
    ...
    if is_android_wrapper and (root / "app/build.gradle").is_file():
        ...
        report.check("Android versionCode 501", "versionCode 501" in gradle)
        report.check("Android versionName 0.5.1", "versionName '0.5.1'" in gradle)
    ...
    if args.log:
        ...
        report.check("device log reproduces id=12 404", 'GET /webview.php/static/index?id=12 HTTP/1.1" 404' in text)
