...
def import_runtime_modules():
    ...
    config_path = Path(tempfile.gettempdir()) / "npps4_v526_validate.toml"
...
async def runtime_checks(main, serial_func, accessory) -> None:
    ...
    # The test resource catalogue must directly grant special accessories in a
    # legal level-1 state while still maximizing common accessories.
    ...
    try:
        created_common, maxed_common, created_special, materials, skipped = await serial_func._grant_accessory_catalogue(
            context2, user, 9999
        )
    finally:
    ...
    require((created_common, maxed_common, created_special, materials, skipped) == (1, 0, 1, 1, 0),
            "test code grants common and dedicated accessories plus materials")

    async def fake_empty_accessory_rows(_context, materials=None):
        return []
    accessory._raw_accessory_rows = fake_empty_accessory_rows
    try:
        empty_result = await serial_func._grant_accessory_catalogue(context2, user, 9999)
    finally:
        accessory._raw_accessory_rows = original_rows
    require(empty_result == (0, 0, 0, 0, 0),
            "empty accessory catalogue returns the full five-field result contract")
    owned_rows = list(session2.execute(sqlalchemy.select(main.UserAccessory)).scalars())
    by_id = {int(row.accessory_id): row for row in owned_rows}
    require(set(by_id) == {10, 20}, "both common and dedicated accessory IDs are granted")
    require((int(by_id[20].exp), int(by_id[20].rank_up_count)) == (0, 0),
            "directly granted dedicated accessory is legal level 1 and unremade")
    require((int(by_id[10].exp), int(by_id[10].rank_up_count)) == (8000, accessory.MAX_RANK_UP_COUNT),
            "directly granted common accessory remains maxed for testing")
...
def static_checks() -> None:
    ...
    require("v5.26-direct-special-accessory-level1-grant-fix" in build,
            "v5.26 build marker is present")
    ...
    require("dedicated_accessories_created" in serial,
            "test result reports directly granted level-1 dedicated accessories")
    require("exp=0" in serial and "rank_up_count=0" in serial,
            "dedicated accessories are directly granted in a legal starter state")
...
def main() -> None:
    ...
    print("v5.26 validation complete")
