#!/usr/bin/env python3
"""NPPS4 v5.04 regression checks for cross-profile friend navigation and Live helpers.

This extends the v5.03 profile/version checks with the social identifier contract:
friend and greeting payloads emit internal account ids, profile routes tolerate old
invite-code references, and Live helper defaults use a real FRIEND_STATUS member.
"""
...
async def exercise_profile_reference_resolution(pyroot: Path, report: Report):
    path = pyroot / "npps4/game/profile.py"
    fn = extract_async(path, "_resolve_profile_user")
    actual = SimpleNamespace(id=2, invite_code="610612777")
    calls: list[tuple[str, int]] = []

    async def get(_context, value):
        calls.append(("id", value))
        return actual if value == 2 else None

    async def find(_context, value):
        calls.append(("invite", value))
        return actual if value == 610612777 else None

    class Error:
        ERROR_CODE_USER_NOT_EXIST = 600
        @staticmethod
        def by_code(code):
    ...
    ns = {
        "__builtins__": builtins.__dict__,
        "user": SimpleNamespace(get=get, find_by_invite_code=find),
        "idol": SimpleNamespace(error=Error),
    }
    resolve = exec_async(fn, path, ns)
    context = SimpleNamespace()
    report.check("profile resolves canonical internal user id", await resolve(context, 2) is actual)
    report.check("canonical id resolution does not query invite code", calls == [("id", 2)], repr(calls))
    calls.clear()
    report.check("profile accepts historical invite-code reference", await resolve(context, 610612777) is actual)
    report.check("invite-code compatibility runs only after id miss", calls == [("id", 610612777), ("invite", 610612777)], repr(calls))


def check_social_identifier_contract(pyroot: Path, report: Report):
    const_source = (pyroot / "npps4/const.py").read_text("utf-8")
    advanced_source = (pyroot / "npps4/system/advanced.py").read_text("utf-8")
    friend_source = (pyroot / "npps4/game/friend.py").read_text("utf-8")
    notice_source = (pyroot / "npps4/game/notice.py").read_text("utf-8")
    profile_source = (pyroot / "npps4/game/profile.py").read_text("utf-8")

    report.check("FRIEND_STATUS defines OTHER=0", "class FRIEND_STATUS" in const_source and "OTHER = 0" in const_source)
    report.check("Live party helper uses a defined neutral friend status", "friend_status = const.FRIEND_STATUS.OTHER" in advanced_source)
    report.check("undefined FRIEND_STATUS.NONE is absent", "FRIEND_STATUS.NONE" not in advanced_source)
    report.check("friend list emits canonical internal account id", "user_id=target.id" in friend_source and "user_id=int(target.invite_code)" not in friend_source)
    report.check("greeting lists emit canonical internal account id", "GreetingUserData(user_id=target.id" in notice_source and "display_user_id = int(target.invite_code)" not in notice_source)
    report.check("all profile reads use the compatibility resolver", profile_source.count("_resolve_profile_user(context, request.user_id)") == 3)
    report.check("profile response canonicalizes returned user id", "user_id=target_user.id" in profile_source and "user_id=request.user_id" not in profile_source)


def check_failure_log(log: Path, report: Report):
    raw = log.read_bytes()
    ...
    report.check("captured Live partyList HTTP 500", 'POST /main.php/live/partyList HTTP/1.1" 500' in text)
    report.check("captured undefined FRIEND_STATUS.NONE traceback", "AttributeError: type object 'FRIEND_STATUS' has no attribute 'NONE'" in text)
    report.check("captured failing profile batch after list/message navigation", "profile/liveCnt status=600" in text and "profile/cardRanking status=600" in text and "profile/profileInfo status=600" in text)
    report.check("captured working profile batch after invite-code search", "profile/liveCnt status=200" in text and "profile/cardRanking status=200" in text and "profile/profileInfo status=200" in text)
...
def main():
    ...
    check_social_identifier_contract(pyroot, report)
    asyncio.run(exercise_profile_reference_resolution(pyroot, report))

    build = (pyroot / "npps4/build_info.py").read_text("utf-8")
    report.check("v5.04 social/live fixes retained in v5.05 build", "v5.05-museum-profile-ranking-fix" in build)
    ...
    if "android-wrapper" in root.name.lower():
        ...
        report.check("Android versionCode advanced beyond v5.04", "versionCode 505" in gradle)
        report.check("Android versionName advanced beyond v5.04", "versionName '0.5.5'" in gradle)
    ...
    if args.log:
        check_failure_log(args.log, report)
