...
async def unlock(
    ...
    *,
    ...
    if not await valid(context, multi_unit_scenario_id):
        return False
    row = await get(context, user, multi_unit_scenario_id)
    if row is None:
        context.db.main.add(
...
async def is_unlocked(context: idol.BasicSchoolIdolContext, user: main.User, multi_unit_scenario_id: int) -> bool:
    return await get(context, user, multi_unit_scenario_id) is not None


async def is_completed(context: idol.BasicSchoolIdolContext, user: main.User, multi_unit_scenario_id: int) -> bool:
    row = await get(context, user, multi_unit_scenario_id)
    return bool(row and row.completed)


async def mark_seen(context: idol.BasicSchoolIdolContext, user: main.User, multi_unit_scenario_id: int) -> bool:
    row = await get(context, user, multi_unit_scenario_id)
...
async def complete(context: idol.BasicSchoolIdolContext, user: main.User, multi_unit_scenario_id: int) -> bool:
    row = await get(context, user, multi_unit_scenario_id)
