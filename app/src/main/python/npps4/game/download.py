...
async def download_update(context: idol.SchoolIdolAuthParams, request: DownloadUpdateRequest) -> DownloadUpdateResponse:
    ...
    profile_download = config.get_profile_download(context.profile)
    if profile_download.backend == "cn_archive":
        links = await download.get_update_files_raw(
            context.request, platform, request.install_version, request.external_version, profile=context.profile
        )
    else:
        try:
        ...
        links = await download.get_update_files(context.request, platform, target_version, profile=context.profile)
    ...
    if config.inject_server_info(context.profile) and profile_download.backend != "cn_archive":
        filehash, size = svinfo.generate_server_info(
            context.request, download.get_server_version(context.profile)
        )
        target_version_str = util.sif_version_string(download.get_server_version(context.profile))
        result.append(
            DownloadUpdateInfo(
...
async def download_batch(context: idol.SchoolIdolAuthParams, request: DownloadBatchRequest) -> DownloadCommonResponse:
    profile_download = config.get_profile_download(context.profile)
    if profile_download.backend == "cn_archive":
        backend = download.get_backend(context.profile)
        expected = download.get_server_version_string(context.profile) if hasattr(backend, "get_server_version_string") else None
    ...
    links = await download.get_batch_files(
        context.request,
        _TARGET_OS_REMAP[request.os.value],
        int(request.package_type),
        request.excluded_package_ids,
...
async def download_additional(
    ...
    links = await download.get_single_package(
        context.request,
        _TARGET_OS_REMAP[request.target_os.value],
        int(request.package_type),
        request.package_id,
        profile=context.profile,
    )
...
async def download_geturl(context: idol.SchoolIdolAuthParams, request: DownloadGetUrlRequest) -> DownloadGetUrlResponse:
    links = await download.get_raw_files(
        context.request, _TARGET_OS_REMAP[request.os.value], request.path_list, profile=context.profile
    )
