...
def build_update_zip(
    ...
    """Build a new 99_0_116 without mutating any operator archive.

    Match the ordinary ZIP metadata observed in the supplied 99_0_113 package.
    The retained content version stays in the existing 113/114/115 config
    entries, so 116 contains only overlay files and deliberately contains no
    client_info/server_info/package_info entry.
    """
    output_zip.parent.mkdir(parents=True, exist_ok=True)
    tmp = output_zip.with_suffix(output_zip.suffix + ".tmp")
    extra_entries = dict(extra_entries or {})
    payloads = {MUSEUM_ENTRY: encrypted_db, **extra_entries}
    with zipfile.ZipFile(template, "r") as src, zipfile.ZipFile(tmp, "w") as dst:
        infos = src.infolist()
        prototype = infos[0] if infos else None
        for filename, data in payloads.items():
            normalized = _normalize(filename)
            if prototype is not None:
                info = _clone_info(prototype, normalized)
                # Real 113/114 use ZIP_STORED.  Keep their extraction behavior
                # and metadata rather than inventing a different package shape.
                info.compress_type = prototype.compress_type
            else:
    ...
    expected = {_normalize(name) for name in payloads}
    with zipfile.ZipFile(output_zip) as check:
        names = {_normalize(n) for n in check.namelist()}
        if check.testzip() is not None or not expected.issubset(names):
            raise RuntimeError(f"generated update ZIP failed validation: expected={sorted(expected)} actual={sorted(names)}")
        if any(name in names for name in DANGEROUS_UPDATE_ENTRIES):
            raise RuntimeError("generated 116 unexpectedly contains a version/server config entry")
...
def build_all(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if template is not None:
        ...
        bundled = Path(__file__).resolve().parents[1] / "assets" / "cn_museum_bridge"
        overlays: dict[str, bytes] = {}
        for name in ("npps4_data_transfer.png", "npps4_manga.png"):
            path = bundled / name
            if path.is_file():
                overlays[f"assets/image/webview/{name}"] = path.read_bytes()
        build_update_zip(template, encrypted_merged, update_zip, overlays)
